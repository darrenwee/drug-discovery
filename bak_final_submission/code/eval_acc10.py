...
def generate_predictions(df_path, model_path, batch_size=150, testing=False, n_channels=4, test_limit=5, validation=False):
    """
    Generates predictions given a csv of protein-ligand pairs, and a model 
    df_path: csv containing paths to protein-ligand pairs, and their protein & ligand ids.
    model_path: model to use
    returns: dict with key of ligand id, value of predictions with index being protein id
    """
...
def _test_submit():
    # for testing the write_predictions_to_file function
    df_path = './data/csv/test_acc10_2_300.csv'
